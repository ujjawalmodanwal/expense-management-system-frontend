...
function Table(props) {
	...
	const fetchTable = async ()=>{
		const {data} = await axios.get(`https://expense-management-system-backend.onrender.com/api/tables/${props.cardData._id}`,
		 {headers:{
	...
	const handleAddFormSubmit = async (event) =>{
		...
		try {
			...
			await axios.post(`https://expense-management-system-backend.onrender.com/api/tables/create/${props.cardData._id}`,{
				newRow
			}, config)
		} catch (error) {
			console.log(error)
	...
	const handleEditFormSubmit = async (event, initialPrice)=>{
		...
		try {
			...
			await axios.put(`https://expense-management-system-backend.onrender.com/api/tables/${editRowId}`,{
				editedRow
			}, config)
		} catch (error) {
			console.log(error)
	...
	const handleDeleteClick = async (rowDataId)=>{
		...
		try {
			...
			await axios.delete(`https://expense-management-system-backend.onrender.com/api/tables/${rowDataId}`, config)
		} catch (error) {
			console.log(error)
		}
		setData(newDatas);
